...
def classification_model_v4():
    ...
    x_F = tf.keras.layers.Flatten()(se_block_out)
    layer = tf.keras.layers.BatchNormalization()(x_F)
    layer = tf.keras.layers.Dense(units=128, activation='relu', kernel_regularizer=tf.keras.regularizers.L1(0.01),
                                   activity_regularizer=tf.keras.regularizers.L2(0.01))(layer)
    layer = tf.keras.layers.Dropout(0.4)(layer)
    classification_fusion = tf.keras.layers.Dense(
        14, activation="sigmoid", name='fusion_global')(layer)
